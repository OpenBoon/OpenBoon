...
class OrganizationControllerTests : MockMvcTest() {
    ...
    fun testGet() {
        ...
        val rsp = mvc.perform(
            MockMvcRequestBuilders.get("/api/v1/organizations/${org.id}")
                .headers(admin())
                .with(SecurityMockMvcRequestPostProcessors.csrf())
                .contentType(MediaType.APPLICATION_JSON_VALUE)
        )
    ...
    fun testSearch() {
        ...
        val rsp = mvc.perform(
            MockMvcRequestBuilders.post("/api/v1/organizations/_search")
                .headers(admin())
                .with(SecurityMockMvcRequestPostProcessors.csrf())
                .contentType(MediaType.APPLICATION_JSON_VALUE)
    ...
    fun testFindOne() {
        ...
        val rsp = mvc.perform(
            MockMvcRequestBuilders.post("/api/v1/organizations/_findOne")
                .headers(admin())
                .with(SecurityMockMvcRequestPostProcessors.csrf())
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .content(Json.serialize(filter))
    ...
    fun testUpdate() {
        ...
        val rsp = mvc.perform(
            MockMvcRequestBuilders.put("/api/v1/organizations/${org.id}")
                .headers(admin())
                .with(SecurityMockMvcRequestPostProcessors.csrf())
                .contentType(MediaType.APPLICATION_JSON_VALUE)
