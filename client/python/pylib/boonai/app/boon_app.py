...
from ..client import BoonAiClient, DEFAULT_SERVER
...
class BoonAiApp:
    ...
    def __init__(self, apikey, server=None):
        ...
        self.client = BoonAiClient(apikey, server or
...
def app_from_env():
    """
    Create a BoonAiApp configured via environment variables. This method
    will not throw if the environment is configured improperly, however
    attempting the use the BoonAiApp instance to make a request
    will fail.
    ...
    Returns:
        BoonAiClient : A configured ZmlpClient
    ...
    return BoonAiApp(apikey, os.environ.get('ZMLP_SERVER'))
