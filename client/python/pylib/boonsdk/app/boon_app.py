...
from ..client import BoonSdkClient, DEFAULT_SERVER

logger = logging.getLogger(__name__)


class BoonSdkApp(object):
    """
    ...
    def __init__(self, apikey, server=None):
        ...
        self.client = BoonSdkClient(apikey, server or
...
def app_from_env():
    """
    Create a BoonSdkApp configured via environment variables. This method
    will not throw if the environment is configured improperly, however
    attempting the use the BoonSdkApp instance to make a request
    will fail.
    ...
    Returns:
        BoonSdkClient : A configured BoonSdkClient
    ...
    return BoonSdkApp(apikey, os.environ.get('BOONAI_SERVER'))
