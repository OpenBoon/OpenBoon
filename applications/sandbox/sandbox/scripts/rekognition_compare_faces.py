...
class Rekognition:
    ...
    def predict(self, source, target):
        """ Compare a face in the source image with each face in the target image

        Args:
            source (str): image path
            target (str): image path
        ...
        # get predictions
        src_json = {'Bytes': source_bytes}
        tgt_json = {'Bytes': target_bytes}
        self.response = self.client.compare_faces(
            SimilarityThreshold=80,
            SourceImage=src_json,
            TargetImage=tgt_json
        )
    ...
    def draw_boxes(self, min_conf=0.01, max_conf=1.00, xr=0, yr=0, image=None):
        ...
        for i, r in enumerate(self.response['FaceMatches']):
            face = r['Face']
            # normalize it's confidence value
            normalized_conf = face['Confidence'] / 100
            ...
            if min_conf <= normalized_conf <= max_conf:
                ...
                cv2.putText(image, f" face{i}, similarity = {r['Similarity']}",
                            org, text_font, 0.7, (255, 0, 0), 2)

        for i, r in enumerate(self.response['UnmatchedFaces']):
            # normalize it's confidence value
            ...
            if min_conf <= normalized_conf <= max_conf:
                ...
                org = (left + width, top + height)
                text_font = cv2.FONT_HERSHEY_SIMPLEX
                cv2.putText(image, f" unmatched face{i}", org, text_font, 0.7, (255, 0, 0), 2)
...
def main():
    ...
    src_asset = search.assets[1]
    src_img = get_proxy_level_path(src_asset, 2)

    rekognition = Rekognition(max_labels=16)
    response = rekognition.predict(target=img, source=src_img)
    ...
    st.sidebar.text(response['SourceImageFace']['Confidence'])
