    ...
    def draw_boxes(self, min_conf=0.01, max_conf=1.00, xr=0, yr=0, image=None):
        ...
        for r in self.response['Labels']:
            ...
            if min_conf <= normalized_conf <= max_conf:
                # check if there are any instances of bounding boxes
                if r['Instances']:
                    # iterate through all bounding boxes found for a detected label
                    for instance in r['Instances']:
                        ...
                        cv2.putText(image, label, org, text_font, 0.7, (255, 0, 0), 2)
