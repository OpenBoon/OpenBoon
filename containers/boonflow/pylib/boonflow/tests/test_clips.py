...
class TestClipTracker(TestCase):
    ...
    def test_clip_tracker_append_dict(self):
    ...
    def test_clip_tracker_append_predictions(self):
        c = ClipTracker(TestAsset(), 'pets')
        p1 = Prediction('mouse', 0.4)
        p2 = Prediction('rat', 0.5)
        c.append_predictions(1, [p1, p2])
        c.append_predictions(2, [p1, p2])
        c.append_predictions(3, [p2])

        tl = c.timeline
        assert 1 == len(tl.tracks['mouse']['clips'])
