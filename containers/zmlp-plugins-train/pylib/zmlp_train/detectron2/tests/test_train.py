...
from zmlp_train.detectron2 import Detectron2TransferLearningTrainer
...
class TensorflowTransferLearningTrainerTests(PluginUnitTestCase):
    ...
    @patch('zmlp_train.detectron2.train.download_dataset', download_dataset)
    @patch.object(DataSetApp, 'get_dataset')
    @patch.object(file_storage.models, 'save_model')
    @patch.object(tempfile, 'mkdtemp')
    @patch.object(DataSetDownloader, 'build')
    def test_process(self, build_patch, temp_patch, upload_patch, data_dataset_patch, labels_patch,
                     model_patch,
                     pub_patch):
        self.prep_assets()
        name = 'custom-flowers-label-detection-tf2-xfer-mobilenet2'
        temp_patch.return_value = zorroa_test_path('coco')
        data_dataset_patch.return_value = DataSet({'id': '12345', 'type': 'LabelDetection'})
        build_patch.return_value = None
        ...
        processor = self.init_processor(Detectron2TransferLearningTrainer(), args)
