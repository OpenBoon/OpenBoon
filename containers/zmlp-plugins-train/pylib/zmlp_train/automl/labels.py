...
from google.cloud import automl_v1beta1 as automl_beta
...
__all__ = [
    ...
    'AutomlVideoLabelDetectionSession'
]
...
class AutomlVideoLabelDetectionSession:
    """
    The AutoMLDataSetImporter handles importing a set of labeled Assets into AutoML.
    This class currently only handle multi-class (single labels) model training, not
    multi-label or objects.
    ...
    credentials = service_account.Credentials.from_service_account_file(
        "/Users/ryangaspar/.google/zorroa-poc-dev-cc537f895317.json"
    )
    ...
    def __init__(self, model, reactor=None):
        ...
        self.app = zmlp.app_from_env()
        self.client = automl_beta.AutoMlClient(credentials=self.credentials)
        self.project_location = self.client.location_path(get_gcp_project_id(), "us-central1")
    ...
    def _train_automl_model(self, dataset):
        ...
        """
        # Leave model unset to use the default base model provided by Google
        metadata = automl_beta.types.VideoObjectTrackingModelMetadata()
        model = automl_beta.types.Model(
            display_name=self.display_name,
            dataset_id=dataset.name,
    ...
    def _create_automl_dataset(self):
        ...
        metadata = automl_beta.types.VideoObjectTrackingDatasetMetadata()
        spec = automl_beta.types.Dataset(
            display_name=self.display_name,
    ...
    def _import_images_into_dataset(self, dataset):
        ...
        gcs_source = automl_beta.types.GcsSource(input_uris=[labels_url])
        input_config = automl_beta.types.InputConfig(gcs_source=gcs_source)
        result = self.client.import_data(dataset.name, input_config).result()
