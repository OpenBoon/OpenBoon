...
from zmlp_train.yolo3.yolo3.model import (
    preprocess_true_boxes,
    yolo_body,
    tiny_yolo_body,
    yolo_loss,
)
from zmlp_train.yolo3.yolo3.utils import get_random_data
...
class YOLOTrainer:
    ...
    def create_model(input_shape, anchors, num_classes,
        ...
        y_true = [Input(
            shape=(h // {0: 32, 1: 16, 2: 8}[l], w // {0: 32, 1: 16, 2: 8}[l], \
                   num_anchors // 3, num_classes + 5)) for l in range(3)]

        model_body = yolo_body(image_input, num_anchors // 3, num_classes)
        logging.debug('Create YOLOv3 model with {} anchors and {} '
                      'classes.'.format(num_anchors, num_classes))

        if load_pretrained:
            model_body.load_weights(weights_path, by_name=True,
                                    skip_mismatch=True)
            logging.debug('Load weights {}.'.format(weights_path))
            if freeze_body in [1, 2]:
                # Freeze darknet53 body or freeze all but 3 output layers.
                num = (185, len(model_body.layers) - 3)[freeze_body - 1]
                for i in range(num):
                    model_body.layers[i].trainable = False
                logging.debug('Freeze the first {} layers of total {} '
                              'layers.'.format(num, len(model_body.layers)))
    ...
    def create_tiny_model(input_shape, anchors, num_classes,
        ...
        y_true = [Input(shape=(h // {0: 32, 1: 16}[l], w // {0: 32, 1: 16}[l], \
                               num_anchors // 2, num_classes + 5)) for l in
                  range(2)]
