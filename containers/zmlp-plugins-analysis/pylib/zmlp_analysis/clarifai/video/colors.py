...
class AbstractClarifaiVideoProcessor(AssetProcessor):
    ...
    def set_analysis(self, extractor, clip_tracker, model):
        ...
        for time_ms, path in extractor:
            ...
            if not concepts:
                continue
            labels = [c['w3c']['name'] for c in concepts]
...
class ClarifaiVideoColorDetectionProcessor(AbstractClarifaiVideoProcessor):
    """ Clarifai color detection"""

    def __init__(self):
        super(ClarifaiVideoColorDetectionProcessor, self).__init__('color-model')

    def init(self):
        self.image_client = ClarifaiColorDetectionProcessor()
