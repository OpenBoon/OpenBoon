...
class OfficerClient(object):
    ...
    def render(self, asset, page, disable_images=False):
        ...
        Returns:
            (str): An internal Boon AI URL where the thumbnails and metadata are located.
        """
        try:
    ...
    def _raise_conn_error(self, rsp):
        ...
        if rsp["status"] == ResponseStatus.FAIL.value:
            raise FatalProcessorException(rsp["message"])
    ...
    def _get_render_request_body(self, asset, page, disable_images):
        ...
        post_files = {
            ...
            'body': json.dumps(body, cls=BoonSdkJsonEncoder)
        }
    ...
    def get_full_storage_prefix(asset):
        ...
        """
        job_storage_path = os.environ.get('BOONAI_JOB_STORAGE_PATH')
        return '{}/officer/{}'.format(job_storage_path, as_id(asset))

    @staticmethod
    def get_file_storage_uri(asset, fname):
        """
        Return a Boon AI file storage URI for the given asset and file name.  The
        Boon AI URI is always stored under the current job. and looks like:

            boonai://job/<jobid>/officer/<asset_id>_<file name>
        ...
        Returns:
            str: The URI.
        """
        jid = os.environ.get('BOONAI_JOB_ID')
        return 'boonai://job/{}/officer/{}_{}'.format(jid, as_id(asset), fname)

    @staticmethod
    def get_metadata_file_id(asset, page):
        """
        Return the Boon AI storage URL for the given page.
        ...
        Returns:
            str: the Boon AI URL to the image.
        """
