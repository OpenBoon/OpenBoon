...
class AwsEnv:
    ...
    def get_rekognition_role_arn():
        ...
        Returns:
            (str): role ARN name
        """
        return os.environ.get('BOONAI_AWS_ML_USER_ROLE_ARN')
    ...
    def get_config():
        return Config(
            region_name=os.environ.get('BOONAI_AWS_REGION', 'us-east-2')
        )

    @staticmethod
    def get_aws_env():
        aws_env = {
            'aws_access_key_id': os.environ.get('BOONAI_AWS_KEY'),
            'aws_secret_access_key': os.environ.get('BOONAI_AWS_SECRET'),
        }
    ...
    @staticmethod
    def get_bucket_name():
        bucket = os.environ.get('BOONAI_AWS_BUCKET')
        if not bucket:
            raise RuntimeError('AWS support is not setup for this environment.')
        return bucket
