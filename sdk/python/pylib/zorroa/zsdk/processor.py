...
from .util.std import import_and_instantiate, as_collection
from .exception import UnrecoverableProcessorException, ZorroaSdkException
...
class Generator(Processor):
    """Base class for Generators.  Generators are responsible for creating Frames.
    """
    def __init__(self):
...
class DocumentProcessor(Processor):
    """Base class for DocumentProcessors. A DocumentProcessor is handed a frame
    which contains the current document.
    """
    ...
    def _process(self, frame):
        ...
        _process is called by the execution engine once for every
        frame that has been generated. The process function is
    ...
    def expand(self, parent_frame, expand_frame, batch_size=None, force=False):
        ...
        """
        if not self.reactor:
            raise ZorroaSdkException("No reactor set on processor")
